...
import type { EmploymentStatus, PayFrequency, PayMethod, PayType } from "@/lib/constants/employment-constants"
...
export async function getPayrollTableDataCore(
  ...
  const payrollRecords = await Payroll.find({
    ...
    .select("employeeId employeeInfo.firstName employeeInfo.lastName hoursWorked.totalHours compensation.payType compensation.payRate earnings.totalGrossPay approvalStatus")
  ...
  const tableData: PayrollTableData[] = employees.map((emp) => {
    ...
    if (existingRecord) {
      ...
      const workingHours = effective?.workingHours ?? emp.currentWorkingHours ?? 40
